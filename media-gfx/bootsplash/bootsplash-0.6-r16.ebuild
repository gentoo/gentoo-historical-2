...
# $Header: /var/cvsroot/gentoo-x86/media-gfx/bootsplash/bootsplash-0.6-r16.ebuild,v 1.1.1.1 2005/11/30 09:37:27 chriswhite Exp $
...
pkg_config() {
	...
	if [ "${KB}" == "2.4" ]; then
		...
		if [ ${KV_MINOR} -ge 25 ]; then
			bzip2 -dc ${ROOT}/usr/share/${PN}/${PN}-3.0.7-2.4.25.patch.bz2 \
					| patch -p1 || die
		elif [ ${KV_MINOR} -ge 22 ]; then
		...
		if [ ${KV_MINOR} -ge 6 ]; then
			bzip2 -dc ${ROOT}/usr/share/${PN}/${PN}-3.1.4-2.6.6-rc2.patch.bz2 \
					| patch -p1 || die
		elif [ ${KV_MINOR} -ge 5 ]; then
			bzip2 -dc ${ROOT}/usr/share/${PN}/${PN}-3.1.4-2.6.5.patch.bz2 \
					| patch -p1 || die
	...
	echo ""
	ewarn "Note that bootsplash will not work with kernels which have CONFIG_REGPARM"
	ewarn "(Processor type and features -> Use register arguments) option enabled."
	ewarn "You will also need framebuffer console support compiled into the kernel"
	ewarn "(and not as a module)."
	echo ""
